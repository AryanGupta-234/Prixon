...
def _snapshot_context(snapshot) -> Dict[str, Any]:
    """Create a compact, decision-relevant world model; never expose raw telemetry wholesale."""
    if snapshot is None:
        return {"available": False}
    return {
        "available": True,
        "age_seconds": round(max(0.0, time.time() - snapshot.timestamp), 1),
        "cpu": {"usage_percent": snapshot.cpu.usage_percent, "cores": snapshot.cpu.core_count,
                "frequency_mhz": snapshot.cpu.frequency_mhz},
        "memory": {"percent": snapshot.memory.percent, "available_mb": snapshot.memory.available_mb,
                    "used_mb": snapshot.memory.used_mb, "total_mb": snapshot.memory.total_mb},
        "disk": {"free_gb": snapshot.disk.free_gb, "used_percent": snapshot.disk.used_percent},
        "network": {"online": snapshot.network.online},
        "battery": {"present": snapshot.battery.present, "percent": snapshot.battery.percent,
                     "charging": snapshot.battery.charging},
        "gpu": {"available": snapshot.gpu.available, "utilization_percent": snapshot.gpu.utilization_percent,
                "vram_used_mb": snapshot.gpu.vram_used_mb, "vram_total_mb": snapshot.gpu.vram_total_mb},
        "top_cpu": [{"name": p.name, "pid": p.pid, "cpu_percent": p.cpu_percent}
                    for p in snapshot.processes.top_by_cpu[:5]],
        "top_memory": [{"name": p.name, "pid": p.pid, "memory_mb": p.memory_mb}
...
def _compact_agent_context(state: AgentState, memory: UnifiedMemory) -> Dict[str, Any]:
    ...
    recent = [{"event": ep.event_type, "target": ep.target_name or ep.target,
               "intent": ep.intent, "success": ep.success} for ep in memory.episodes[-8:]]
    return {
...
def route(user_text: str, candidates: List[Dict], state: AgentState, memory: UnifiedMemory,
    ...
    if tier1.resolved:
        ...
        if target:
            return RoutedResult(NLUResult(
                ...
                "tier1", {"reference": tier1.reference, "confidence": tier1.confidence,
                           "reason": tier1.reason})

    t2 = tier2.classify(user_text, candidates)
    if t2.resolved:
        result = NLUResult(match_target=t2.target, confidence="high", reply="On it.",
                           intent=t2.intent or "unknown", parameters=t2.parameters or {}, reference="none",
                           raw={"tier2_reason": t2.reason})
        return RoutedResult(result, "tier2-lexical", {"tier1_reason": tier1.reason,
                                                        "confidence": t2.confidence, "reason": t2.reason})
    ...
    if semantic_index is not None and semantic_index.ready:
        ...
        if t2_semantic.resolved:
            result = NLUResult(match_target=t2_semantic.target, confidence="high", reply="On it.",
            ...
            return RoutedResult(result, "tier2-semantic", {"tier1_reason": tier1.reason,
                                                             "tier2_lexical_reason": t2.reason,
                                                             "confidence": t2_semantic.confidence,
                                                             "reason": t2_semantic.reason})
    ...
    return RoutedResult(result, "tier3", {"raw": result.raw, "tier1_reason": tier1.reason,
                                           "tier2_lexical_reason": t2.reason,
